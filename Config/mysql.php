	...
	if($conecta->connect_error){
		...
		if($row[0]>1){
			if($conecta->query("DELETE FROM usuario WHERE nombre = '$user' AND encuesta = '$encuesta';") == TRUE){
				escribir("login", "Cerró Sesión: (TODAS)  ".$user." ". $sesion ." ".$encuesta );
				escribir("prueba", "rows  ".$user." ". $sesion ." ".$encuesta." ".$row[0] );
				...
				header("Location: http://$host/bancoestado-servidores/index.php");
			}else{
				echo $conecta->error;
			}
		}else if($row[0]<1){
