...
function getResponsable($user){
	...
	$query = "SELECT * FROM banco.aplicativos WHERE ID in (SELECT ID_APLICATIVO FROM banco.subaplicativos WHERE RA = '$user')";//SELECT * FROM banco.procesos WHERE RESPONSABLE = '$user'";
...
include("../API/mysqlconnection.php");
	$query = "SELECT * FROM banco.subaplicativos WHERE ID = $id AND RA ='$user'";//SELECT * FROM banco.procesos WHERE RESPONSABLE = '$user'";
...
function revisado($id){
	...
	$query = "UPDATE banco.subaplicativos SET R_RA = 1, RA_MODIFICADO = '$hoy'  WHERE ID = $id";
